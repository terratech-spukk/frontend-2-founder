...
export default function Page() {
  ...
  const [method, setMethod] = useState<"card" | "promptpay" | "linepay">(
    "card",
  );
    ...
    try {
      ...
      if (data?.error === "Account expired") {
        addToast({
          title: "บัญชีหมดอายุ",
          description:
            "Your account has expired. Please contact support or login again.",
          color: "warning",
        });
      ...
      <main className="relative z-10 flex min-h-screen items-center justify-center p-6">
        <div className="w-full max-w-xl rounded-2xl border border-white/20 bg-white/10 p-6 backdrop-blur-xl shadow-2xl">
          ...
          <div className="text-center mb-4">
            <div className="text-sm tracking-widest text-amber-200">
              TERRATECH
            </div>
                  ...
                  >
                    <div className="grid grid-cols-3 items-center">
                      ...
                      <div className="text-right">
                        {formatTHB(item.quantity * item.price).replace("฿", "")}
                      </div>
                    </div>
                    {item.note && (
            ...
            {method === "card" ? (
              ...
              <div className="flex items-center gap-3 text-white/90">
                ...
                <span className="text-sm">
                  เลือก {method === "promptpay" ? "PromptPay" : "LINE Pay"}
                </span>
              </div>
            )}
