      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-2 sm:space-x-5">
            <h1
              onClick={handleMenu}
              className="text-white text-lg sm:text-xl font-bold cursor-pointer hover:text-gray-300 transition-colors"
            >
              TERRA
            </h1>
            <h1
              onClick={handleMenu}
              className="text-white text-lg sm:text-xl font-bold cursor-pointer hover:text-gray-300 transition-colors hidden sm:block"
            >
              Menu
            </h1>
          </div>
            ...
            {user?.role === "admin" && (
              <>
                <div
                  ...
                  className="text-white text-lg font-bold cursor-pointer underline hover:text-gray-300 transition-colors"
                >
                  Order Dashboard
                </div>
                <div
              ...
              <div className="hidden sm:flex items-center space-x-2">
                <div className="text-white">
                  ...
                  <span className="text-xs sm:text-sm opacity-75 ml-1 sm:ml-2">
                    ({user.role}){user.room_id && ` - Room ${user.room_id}`}
                  </span>
                </div>
                <button
        ...
        <div
          id="mobile-menu"
          className="hidden lg:hidden border-t border-white/20 py-4"
        >
          <div className="flex flex-col space-y-3">
            <div
              onClick={handleMenu}
              className="text-white text-lg font-bold cursor-pointer hover:text-gray-300 transition-colors px-2 py-1"
            >
              Menu
            </div>
            {user?.role === "admin" && (
              <>
                <div
                  onClick={handleDashboard}
                  className="text-white text-lg font-bold cursor-pointer underline hover:text-gray-300 transition-colors px-2 py-1"
                >
                  Order Dashboard
                </div>
                <div
            ...
            {user ? (
              ...
              <div className="px-2 py-1">
                <span className="text-white text-sm opacity-75 block mb-2">
                  Not logged in
                </span>
                <button
                  onClick={() => router.push("/login")}
