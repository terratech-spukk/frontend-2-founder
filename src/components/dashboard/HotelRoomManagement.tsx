...
export function HotelRoomManagement({ onError }: HotelRoomManagementProps) {
  ...
  const handleReserve = async (
    ...
    try {
          ...
          room.room_number === roomNumber
            ? {
                ...
                qr_code_data: JSON.stringify(
                  responseData.credentials.qrCodeData,
                ),
              }
            : room,
        ),
      ...
      const apiError = error as ApiError;
      const errorMessage =
        apiError?.message || "Failed to reserve the room. Please try again.";
  ...
  const handleCheckin = async (roomNumber: number) => {
    try {
      ...
      const apiError = error as ApiError;
      const errorMessage =
        apiError?.message || "Failed to check in the room. Please try again.";
  ...
  const handleCheckout = async (roomNumber: number) => {
    try {
      ...
      const apiError = error as ApiError;
      const errorMessage =
        apiError?.message || "Failed to check out the room. Please try again.";
